...
def line_fit(binary_warped):
	...
	color_for_rectangle = (0, 255, 0)

	# Step through the windows one by one
	for window in range(nwindows):
		# Identify window boundaries in x and y (and right and left)
		##TO DO
		winylow = binary_warped.shape[0] - (window + 1) * window_height
		winyhigh = binary_warped.shape[0] - window*window_height

		win_xleft_low = max(0, leftx_current - margin) 
		win_xleft_high = leftx_current + margin
		win_xright_low = rightx_current - margin
		win_xright_high = min(rightx_current + margin, binary_warped.shape[1])

		####
		# Draw the windows on the visualization image using cv2.rectangle()
		##TO DO
		
		left_start = np.array([win_xleft_low, winylow])
		left_end = np.array([win_xleft_high, winyhigh])
		print(type(left_start), type(left_end), type(color_for_rectangle))
		right_start = np.array([win_xright_low, winylow])
		right_end = np.array([win_xright_high, winyhigh])
		out_img = cv2.rectangle(out_img, tuple(left_start), tuple(left_end), (0,255,0), 1)
		out_img = cv2.rectangle(out_img, tuple(right_start), tuple(right_end), (0,0,255), 1)
		####
		# Identify the nonzero pixels in x and y within the window
		##TO DO
		def filter_indices(x, y, x_low, x_high, y_low, y_high):
			return (x < x_high) & (x > x_low) & (y < y_high) & (y > y_low)
		left_inds1 = filter_indices(nonzerox, nonzeroy, win_xleft_low, win_xleft_high, winylow, winyhigh)
		right_inds1 = filter_indices(nonzerox, nonzeroy, win_xright_low, win_xright_high, winylow, winyhigh)

		left_inds = np.nonzero(left_inds1)[0]
		right_inds = np.nonzero(right_inds1)[0]
